...
note-finance 移植版: source_id/author_id キー、schema_version 2.2 に適合。
"""
...
class TestMapAcademicPapersSources:
    ...
    def test_正常系_単一論文でSourceとAuthorsとAUTHORED_BYが生成される(
        ...
        assert source["authority_level"] == "academic"
    ...
    def test_正常系_SourceのIDがgenerate_source_idと一致する(self) -> None:
        ...
        assert result["sources"][0]["source_id"] == expected_id
...
class TestMapAcademicPapersAuthors:
    ...
    def test_正常系_AuthorのIDがgenerate_author_idと一致する(self) -> None:
        ...
        assert result["authors"][0]["author_id"] == expected_id
...
class TestMapAcademicPapersAuthoredBy:
    ...
    def test_正常系_AUTHORED_BYのfrom_idがSourceでto_idがAuthor(self) -> None:
        ...
        assert rel["from_id"] == result["sources"][0]["source_id"]
        assert rel["to_id"] == result["authors"][0]["author_id"]
...
class TestMapAcademicPapersCites:
    ...
    def test_正常系_existing_source_idsに含まれる引用先のみCITESが生成される(
        ...
        paper = _make_paper(
            references=[
                ...
                {"title": "Unknown Ref", "arxiv_id": "9999.99999", "s2_paper_id": None},
            ]
        )
    ...
    def test_正常系_バッチ内の論文間CITESも生成される(self) -> None:
        paper1 = _make_paper(arxiv_id="2301.00001", references=[])
        paper2 = _make_paper(
            arxiv_id="2301.00002",
            references=[
                {"title": "Paper 1", "arxiv_id": "2301.00001", "s2_paper_id": None}
            ],
        )
...
class TestMapAcademicPapersCoauthoredWith:
    ...
    def test_正常系_2著者で1ペアのCOAUTHORED_WITHが生成される(self) -> None:
        data: dict[str, Any] = {
            "papers": [_make_paper(authors=[{"name": "Alice"}, {"name": "Bob"}])],
            "existing_source_ids": [],
        }
...
class TestMapAcademicPapersEdgeCases:
    ...
    def test_エッジケース_空リストで空キューが返される(self) -> None:
        data: dict[str, Any] = {"papers": [], "existing_source_ids": []}
    ...
    def test_エッジケース_arxiv_idがない論文はスキップされる(self) -> None:
        ...
        data: dict[str, Any] = {"papers": [paper], "existing_source_ids": []}
    ...
    def test_正常系_schema_versionが2_2(self) -> None:
        data: dict[str, Any] = {"papers": [], "existing_source_ids": []}
