"""Unit tests for Networking CompanyConfig definitions and snapshot selector extraction.

Validates that:
1. All 2 Networking companies have correct CompanyConfig definitions
2. CSS selectors extract articles from HTML snapshots correctly
3. NETWORKING_COMPANIES list is complete and well-formed
"""
...
from rss.services.company_scrapers.configs.networking import (
    CISCO,
    JUNIPER_NETWORKS,
...
_SNAPSHOTS_DIR = Path(__file__).parent.parent / "snapshots" / "networking"
"""Directory containing HTML snapshots for Networking companies."""

_EXPECTED_COMPANY_COUNT = 2
"""Number of Networking companies expected."""
...
# ---------------------------------------------------------------------------
# NETWORKING_COMPANIES list
# ---------------------------------------------------------------------------


class TestNetworkingCompaniesList:
    """Tests for the NETWORKING_COMPANIES list."""

    def test_正常系_2社全てが含まれている(self) -> None:
        assert len(NETWORKING_COMPANIES) == _EXPECTED_COMPANY_COUNT

    def test_正常系_全要素がCompanyConfig型(self) -> None:
        for config in NETWORKING_COMPANIES:
            assert isinstance(config, CompanyConfig)

    def test_正常系_キーが一意(self) -> None:
        keys = [c.key for c in NETWORKING_COMPANIES]
        assert len(keys) == len(set(keys))

    def test_正常系_全てカテゴリがnetworking(self) -> None:
        for config in NETWORKING_COMPANIES:
            assert config.category == "networking"
    ...
    def test_正常系_リスト内容が個別定数と一致(self) -> None:
        assert NETWORKING_COMPANIES == _ALL_CONFIGS
    ...
    def test_正常系_期待される企業キーが全て存在する(self) -> None:
        ...
        actual_keys = {c.key for c in NETWORKING_COMPANIES}
...
class TestSpecificCompanyConfigs:
    ...
    def test_正常系_CiscoのティッカーがCSCO(self) -> None:
        assert CISCO.investment_context.tickers == ("CSCO",)

    def test_正常系_JuniperNetworksのティッカーがJNPR(self) -> None:
        assert JUNIPER_NETWORKS.investment_context.tickers == ("JNPR",)
    ...
    def test_正常系_Ciscoのblog_urlが正しい(self) -> None:
        assert CISCO.blog_url == "https://newsroom.cisco.com/"

    def test_正常系_JuniperNetworksのblog_urlが正しい(self) -> None:
        assert JUNIPER_NETWORKS.blog_url == "https://newsroom.juniper.net/"
...
class TestConfigsPackageImport:
    ...
    def test_正常系_configsパッケージからインポートできる(self) -> None:
        from rss.services.company_scrapers.configs import NETWORKING_COMPANIES

        assert len(NETWORKING_COMPANIES) == _EXPECTED_COMPANY_COUNT

    def test_正常系_networkingモジュールから全定数をインポートできる(self) -> None:
        from rss.services.company_scrapers.configs.networking import (
            CISCO,
            JUNIPER_NETWORKS,
            NETWORKING_COMPANIES,
        ...
        assert all_names == NETWORKING_COMPANIES
