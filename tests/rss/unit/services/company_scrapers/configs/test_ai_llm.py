"""Unit tests for AI/LLM CompanyConfig definitions and snapshot selector extraction.

Validates that:
1. All 11 AI/LLM companies have correct CompanyConfig definitions
2. CSS selectors extract articles from HTML snapshots correctly
3. AI_LLM_COMPANIES list is complete and well-formed
"""
...
from rss.services.company_scrapers.configs.ai_llm import (
    AI_LLM_COMPANIES,
    ANTHROPIC,
    COHERE,
    DEEPMIND,
...
_SNAPSHOTS_DIR = Path(__file__).parent.parent / "snapshots" / "ai_llm"
"""Directory containing HTML snapshots for AI/LLM companies."""

_EXPECTED_COMPANY_COUNT = 11
"""Number of AI/LLM companies expected."""
...
_CONFIG_SNAPSHOT_MAP: dict[str, str] = {
    "openai": "openai.html",
    "deepmind": "deepmind.html",
    "meta_ai": "meta_ai.html",
    "anthropic": "anthropic.html",
    "microsoft_ai": "microsoft_ai.html",
    "xai": "xai.html",
    "mistral_ai": "mistral_ai.html",
    "cohere": "cohere.html",
    "stability_ai": "stability_ai.html",
    "perplexity_ai": "perplexity_ai.html",
    "inflection_ai": "inflection_ai.html",
}
...
# ---------------------------------------------------------------------------
# AI_LLM_COMPANIES list
# ---------------------------------------------------------------------------


class TestAiLlmCompaniesList:
    """Tests for the AI_LLM_COMPANIES list."""

    def test_正常系_11社全てが含まれている(self) -> None:
        assert len(AI_LLM_COMPANIES) == _EXPECTED_COMPANY_COUNT

    def test_正常系_全要素がCompanyConfig型(self) -> None:
        for config in AI_LLM_COMPANIES:
            assert isinstance(config, CompanyConfig)

    def test_正常系_キーが一意(self) -> None:
        keys = [c.key for c in AI_LLM_COMPANIES]
        assert len(keys) == len(set(keys))

    def test_正常系_全てカテゴリがai_llm(self) -> None:
        for config in AI_LLM_COMPANIES:
    ...
    def test_正常系_リスト内容が個別定数と一致(self) -> None:
        assert AI_LLM_COMPANIES == _ALL_CONFIGS
    ...
    def test_正常系_期待される企業キーが全て存在する(self) -> None:
        ...
        actual_keys = {c.key for c in AI_LLM_COMPANIES}
...
class TestSpecificCompanyConfigs:
    ...
    def test_正常系_OpenAIのティッカーがMSFT(self) -> None:
        assert OPENAI.investment_context.tickers == ("MSFT",)

    def test_正常系_DeepMindのティッカーがGOOGL(self) -> None:
        assert DEEPMIND.investment_context.tickers == ("GOOGL",)

    def test_正常系_MetaAIのティッカーがMETA(self) -> None:
        assert META_AI.investment_context.tickers == ("META",)

    def test_正常系_AnthropicのティッカーがAMZNとGOOGL(self) -> None:
        assert "AMZN" in ANTHROPIC.investment_context.tickers
        assert "GOOGL" in ANTHROPIC.investment_context.tickers

    def test_正常系_MicrosoftAIのティッカーがMSFT(self) -> None:
        assert MICROSOFT_AI.investment_context.tickers == ("MSFT",)

    def test_正常系_xAIのティッカーが空(self) -> None:
        assert XAI.investment_context.tickers == ()

    def test_正常系_InflectionAIのティッカーがMSFTとNVDA(self) -> None:
        assert "MSFT" in INFLECTION_AI.investment_context.tickers
        assert "NVDA" in INFLECTION_AI.investment_context.tickers

    def test_正常系_PerplexityAIはPlaywright必須(self) -> None:
        assert PERPLEXITY_AI.requires_playwright is True

    def test_正常系_PerplexityAIのrate_limitが5秒(self) -> None:
        assert PERPLEXITY_AI.rate_limit_seconds == 5.0

    def test_正常系_OpenAIのblog_urlが正しい(self) -> None:
        assert OPENAI.blog_url == "https://openai.com/news/"

    def test_正常系_AnthropicのURLがresearch(self) -> None:
        assert ANTHROPIC.blog_url == "https://anthropic.com/research"
...
class TestConfigsPackageImport:
    ...
    def test_正常系_configsパッケージからインポートできる(self) -> None:
        from rss.services.company_scrapers.configs import AI_LLM_COMPANIES

        assert len(AI_LLM_COMPANIES) == _EXPECTED_COMPANY_COUNT

    def test_正常系_ai_llmモジュールから全定数をインポートできる(self) -> None:
        from rss.services.company_scrapers.configs.ai_llm import (
            AI_LLM_COMPANIES,
            ANTHROPIC,
            COHERE,
        ...
        assert all_names == AI_LLM_COMPANIES
