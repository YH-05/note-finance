...
```bash
cat articles/{article_id}/article-meta.json | python -c "import sys,json; d=json.load(sys.stdin); print(d.get('workflow',{}).get('research',{}).get('decisions','not_done'))"
```

### 2. 初稿作成

`finance-article-writer` の処理として、`decisions.json`, `sources.json`, `claims.json`, `article-meta.json` を入力に `02_edit/first_draft.md` を生成する。
...
`finance-reviser` として `first_draft.md`, `critic.json`, `sources.json` を入力に `02_edit/revised_draft.md` を生成。
