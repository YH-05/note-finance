...
**課題**:
- フォルダ構成が `{category}_{seq}_{theme}/`, `asset_management/{slug}/`, `exp-{theme}-{seq}/`, `market_report/{date}/` の4+パターン混在
...
# === Core (必須) ===
title: "記事タイトル"
category: asset_management      # 6カテゴリから選択
type: column                   # column | case_study | experience | market_report | data_analysis
...
theme: ""                      # asset_management / side_business
...
template/
  _common/                  # 全カテゴリ共通（新規作成）
    ...
    03_published/
      article.md
  asset_management/          # 旧 asset_management + investment_education
    01_research/
...
| 6 | `asset_management/fund_selection_age_based/` | `asset_management/2026-03-08_fund-selection-age-based/` | カテゴリ変更, 01_research/03_published追加 |
| 7 | `asset_management/index-investing-...` | `asset_management/2026-03-06_index-investing-portfolio/` | 同上 |
| 8 | `asset_management/index_vs_etf_2026/` | `asset_management/2026-03-08_index-vs-etf-2026/` | フラット→構造化 |
...
| 11 | `market_report/2026-02-23/` | `market_report/2026-02-23_weekly-market-report/` | data→01_research/market, 02_edit→02_draft |
...
- `topic-suggester.md`: articles/ スキャン更新
...
- `template/asset_management/` → `template/asset_management/` リネーム
- `template/economic_indicators/` → `template/macro_economy/` リネーム
- `template/market_report/` → `template/market_report/` リネーム
- `template/investment_education/` → `template/asset_management/` に統合
...
4. **E2Eテスト**: `/article-full` で asset_management カテゴリの記事を1本通しで作成（init→research→draft→critique→publish(dry-run)）
