...
現在の `topic-suggester`（LLM知識のみ）に WebSearch ベースのトレンドリサーチを追加。
...
| `.claude/agents/topic-suggester.md` | インライン評価基準を `references/scoring-rubric.md` 参照に置換。トピックソース定義を削除し、検索結果を入力として受け取る形に簡素化 |
