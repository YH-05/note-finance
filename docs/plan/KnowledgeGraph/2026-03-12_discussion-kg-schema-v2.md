...
- [ ] `scripts/emit_research_queue.py` の pdf-extraction マッパーを v2 対応に更新 — Chunk/FinancialDataPoint/FiscalPeriod ノード追加、Fact/Claim 分離 (優先度: 高)
- [ ] PDFパイプライン Step 5 (pipeline統合) の実装 (優先度: 高)
- [ ] PDFパイプライン Step 6 (Neo4j投入) の実装 — save-to-research-graph スキル活用 (優先度: 高)
