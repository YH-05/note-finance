...
| 状況 | PROJECT_ROOT | .env読み込み |
|------|--------------|--------------|
| editable install (-e) | /path/to/finance/ | ✅ 成功 |
| 通常 install (site-packages) | .venv/lib/python3.12/ | ❌ 失敗 |
...
1. **カレントディレクトリ優先の`.env`探索**: `.env`ファイルの探索順序を変更し、site-packages環境でも動作するようにする
2. **デフォルトパスの環境変数化**: データディレクトリのパスを環境変数で設定可能にし、`__file__`ベースのパスはフォールバックとして残す
...
1. `src/utils_core/settings.py` に `_find_env_file()` 追加、`load_project_env()` 修正
2. `src/database/db/connection.py` に `get_data_dir()` 追加
3. `src/market/fred/historical_cache.py` の `get_default_cache_path()` 修正
4. `src/market/fred/fetcher.py` に `_get_default_presets_path()` 追加
5. `.env.example` 更新
6. `make check-all` で品質確認
