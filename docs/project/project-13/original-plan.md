...
wealth blog スクレイピング（`scrape_wealth_blogs.py`）と topic-discovery スキルが収集した情報を article-neo4j（bolt://localhost:7689）に蓄積するパイプラインが未整備。既存の `emit_research_queue.py` + `save-to-research-graph` パターンを拡張し、2つの新コマンドマッパーとオーケストレータースキルを追加する。
...
| # | 変更 | 理由 |
|---|------|------|
| 1 | **Author サポートを除外** | 全239ファイルの `author` が空文字。save-to-research-graph も AUTHORED_BY リレーション未対応。デッドコードになるため後回し |
...
2. `NEO4J_URI=bolt://localhost:7689 /save-to-research-graph --source {command}`
...
NEO4J_URI=bolt://localhost:7689 NEO4J_PASSWORD=gomasuke \
  /save-to-research-graph --source topic-discovery --dry-run
