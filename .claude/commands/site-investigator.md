...
ハイブリッド方式: 調査は browser-use CLI 2.0（トークン効率優先）、検証は Playwright MCP。
...
1. `.claude/skills/site-investigator/SKILL.md` を読み込み、5 Phase の調査プロトコルに従って実行する
   - **Phase 1-5（調査）**: browser-use CLI 2.0 (`source ~/.browser-use-env/bin/activate && browser-use`) を Bash 経由で使用
