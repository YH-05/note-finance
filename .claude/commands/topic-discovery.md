...
4. **topic-discovery スキルの実行**
   ...
   **処理順序（KGファースト）**:
   1. **Phase 0**: research-neo4j から知識ギャップ・未活用エンティティ・トレンドをマイニング
   2. **[HF0]**: KG由来のトピック候補をユーザーに提示し、Web検索の要否を確認
   3. **Phase 1**: ユーザーが承認した場合のみWeb検索を実行（または特定領域に絞って実行）
   4. **Phase 3**: KG補正付きスコアリングで最終提案を生成

   **注意**: `--no-search` オプションが指定された場合は、[HF0] をスキップして
   KGデータのみでスコアリングを実行する（最速モード）。
