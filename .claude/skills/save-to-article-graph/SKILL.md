...
/save-to-article-graph (このスキル = オーケストレーター)
  ...
  +-- Phase 2: Neo4j 投入（save-to-graph スキルのロジック）
        +-- NEO4J_URI=bolt://localhost:7688 で research-neo4j に接続
...
save-to-graph スキルのロジックを使用して、graph-queue JSON を research-neo4j に投入する。

**重要**: `NEO4J_URI` を `bolt://localhost:7688` に設定して research-neo4j に接続する。

### 前提条件チェック

```bash
# research-neo4j コンテナの起動確認
CONTAINER_STATUS=$(docker inspect research-neo4j --format='{{.State.Status}}' 2>/dev/null)
```
...
research-neo4j が利用可能になったら、以下のコマンドで手動投入できます:
  NEO4J_URI=bolt://localhost:7688 /save-to-graph --file {GQ_FILE}
```
...
| graph-queue ファイル数 | {gq_count} |
| Phase 1 | OK |
| Phase 2 | SKIP (research-neo4j 未起動) |

### 手動投入コマンド

research-neo4j 起動後に以下を実行:

```bash
NEO4J_URI=bolt://localhost:7688 /save-to-graph --source {command}
```
```
...
| 変数名 | デフォルト | 説明 |
|--------|-----------|------|
| NEO4J_URI | bolt://localhost:7688 | research-neo4j Bolt URI（本スキルが自動設定） |
...
- グレースフルデグラデーション対応（research-neo4j 未起動時は Phase 2 スキップ）
