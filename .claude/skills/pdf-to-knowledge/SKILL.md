...
SKILL_RUN_ID=$(python3 scripts/skill_run_tracer.py start \
    --skill-name pdf-to-knowledge \
    --command-source "/pdf-to-knowledge" \
    --input-summary "Processing ${pdf_path} (${PAGE_COUNT:-unknown} pages)")
```
...
python3 scripts/skill_run_tracer.py complete \
    ...
    --status success \
    --output-summary "Phase 1-4 complete: ${PAGE_COUNT} pages, ${CHUNK_COUNT} chunks, ${EXTRACTION_STATS}"
```

### 実行完了時（部分成功 — Phase 1-3 のみ完了時）

```bash
python3 scripts/skill_run_tracer.py complete \
    --skill-run-id "$SKILL_RUN_ID" \
    --status partial \
    --output-summary "Phase 1-${LAST_PHASE} complete: ${PAGE_COUNT} pages, ${CHUNK_COUNT} chunks" \
    --error-message "Phase ${FAILED_PHASE}: ${ERROR_MSG}" \
    --error-type "${ERROR_TYPE}"
```

### 実行完了時（エラー — Phase 1 で失敗時）

```bash
python3 scripts/skill_run_tracer.py complete \
    --skill-run-id "$SKILL_RUN_ID" \
    --status failure \
    --error-message "Phase 1: ${ERROR_MSG}" \
    --error-type "${ERROR_TYPE}"
```
...
| error_type | 説明 |
|------------|------|
| pdf_not_found | PDF ファイル不存在（E001） |
| pdf_invalid | 拡張子不正・ファイルサイズ 0（E002, E003） |
| pdf_read_failure | PDF 読込失敗（E004） |
| markdown_conversion | Markdown 変換失敗・結果空（E005, E006） |
| knowledge_extraction | 知識抽出失敗（Phase 2） |
| graph_queue_generation | graph-queue 生成失敗（Phase 3） |
| neo4j_connection | Neo4j 接続失敗（Phase 4） |
| cypher_execution | Cypher 実行エラー（Phase 4） |
...
### 2026-03-18: Observability セクション追加（Issue #190）

- Phase 1 前に skill_run_tracer start スニペットを挿入
- Phase 4 後に skill_run_tracer complete スニペットを挿入（成功/部分成功/エラーの 3 パターン）
- error_type 分類表を追加（8 種類: pdf_not_found, pdf_invalid, pdf_read_failure, markdown_conversion, knowledge_extraction, graph_queue_generation, neo4j_connection, cypher_execution）
- 関連リソースに skill_run_tracer を追加
