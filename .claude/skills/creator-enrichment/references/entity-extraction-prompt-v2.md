...
**Story 優先判定ルール**:
以下のいずれかを含む場合、Tip ではなく **Story** に分類すること。
- 「〜してみた」「〜した結果」「〜ヶ月目の報告」
- 具体的な金額・期間の before/after（「月収3万円→月収30万円」等）
- 失敗談・反省点の記述（「失敗した」「後悔している」「次はこうする」）
- 一人称視点での時系列ストーリー
- 「my experience」「how I started」「first year results」
...
- 汎用的すぎるものは避ける（「ビジネス」「成功」「選び方」「規模別」→ 抽出しない）

### 語順正規化ルール（必須）

「A・B」「A×B」のような複合 Concept は、**辞書順（Unicode コードポイント順）に統一**すること。
これにより「副業・起業」と「起業・副業」のような重複を防ぐ。

| 正規化前（NG） | 正規化後（OK） | 理由 |
|---------------|---------------|------|
| 起業・副業 | 副業・起業 | 「副」< 「起」（Unicode 順） |
| スピリチュアル・占い | 占い・スピリチュアル | 「占」< 「ス」 |
| リセール・フリマ | フリマ・リセール | 「フ」< 「リ」 |
| 占いInstagram | Instagram占い | 英字を先頭に |

**ルール**:
1. 区切り文字（`・`、`×`、`/`）で分割
2. 各部分を Unicode コードポイント順にソート
3. 元の区切り文字で再結合
4. 英語の場合は大文字小文字を統一（Title Case）
5. 英語 Concept の単数形/複数形は**単数形に統一**（Payments → Payment）
...
コンテンツ分類後、以下のジャンル適合性を検証してください。

1. コンテンツの主題が、入力ジャンルのターゲット読者に**固有**か？
2. 別のジャンルの方が明らかに適切な場合は `genre_override` を設定
3. 複数ジャンルに関連する場合は `cross_genre_tags` に他ジャンルのタグを追加

判定基準:
- career: 副業・転職・フリーランス・収入に直接関連
- beauty-romance: 美容・コスメ・恋愛・婚活に直接関連
- spiritual: 占い・タロット・スピリチュアル・開運に直接関連
- SNSマーケティング・EC戦略・市場データ等の汎用トピック → 元ジャンル維持、cross_genre_tags 追加

出力に以下を追加:
```json
{
  "genre_override": null,
  "cross_genre_tags": ["marketing", "market-data"]
}
```

`genre_override` が設定された場合、emit_creator_queue_v2.py は IN_GENRE のターゲットを変更する。
