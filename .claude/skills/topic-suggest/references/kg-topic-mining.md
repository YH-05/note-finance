...
topic-suggest の Phase 0（KG照会でトピック発掘）で使用する。
`mcp__neo4j-research__research-read_neo4j_cypher` で実行すること（読み取り専用）。

> **注**: 本ファイルは旧 `/topic-discovery` から移植した KG マイニングロジック。
> 個別エンティティラベル（`Company|Technology|...`）と KG v3 スキーマに準拠している。
> own_article（株投資ラボ自身の記事）専用クエリは `kg-own-article-mining.md` 参照。
