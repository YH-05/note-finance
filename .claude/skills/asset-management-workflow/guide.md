...
> **重要**: X投稿は `x-post-generator` スキル（`/x-post`）で生成する。
> asset-management-writer エージェントではnote記事のみ生成し、X投稿はスキルに委譲する。

```
/x-post @{article_dir}
```

x-post-generator スキルが以下を自動処理する:
- `meta.yaml` の `target_audience` に基づく層別テンプレート適用（beginner/intermediate/advanced）
- カテゴリ × 層に最適なフックパターン自動選択（A逆説/B疑問形/C数字/D宣言）
- 参照ライブラリから類似サンプル検索・活用
- 280字以内の文字数チェック・調整
- カテゴリ別ハッシュタグと `{URL}` プレースホルダーの自動挿入

詳細: `.claude/skills/x-post-generator/SKILL.md`
