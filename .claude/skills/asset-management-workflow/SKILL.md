...
※ X投稿（02_draft/x_post.md）は Step 2.2 で `/x-post` スキルにより別途生成
"""
)
```

### ステップ2.2: X投稿生成（x-post-generator スキル）

note記事の初稿完成後、`/x-post @{article_dir}` を実行してX投稿を生成する。
x-post-generator スキルが `meta.yaml` の `target_audience` に基づいて
層別テンプレート・参照ライブラリを自動適用する。
...
| `02_draft/x_post.md` | X投稿（`/x-post` スキルで生成） | 280字以内 |
...
| X投稿生成スキル | `.claude/skills/x-post-generator/SKILL.md` |
