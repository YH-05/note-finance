---
name: review-pr
description: PRレビュー（コード品質・セキュリティ・テスト）。7つのサブエージェントを並列実行してPRを多角的にレビューし、フィードバックを提供する。
...
7つのサブエージェントを並列実行し、コード品質・セキュリティ・テストの3つの観点から包括的なレビューを実施します。
...
- `.claude/agents/pr-*/`: 7つのPRレビューエージェント
