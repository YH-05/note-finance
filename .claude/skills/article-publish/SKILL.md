...
- [ ] ハッシュタグが記事末尾に含まれているか
- [ ] **タイトル**: 一発で内容が理解でき、読者メリットが明確か（common-rules.md § 3 参照）
- [ ] **タイトル**: 全角30-38字以内に収まっているか
- [ ] **文体**: ダッシュ記号（`ーー` `——` `—` `--`）が本文・タイトルに残っていないか
- [ ] **記事末尾**: 挨拶文（`snippets/closing-greeting.md` の全文: `いつも読んでいただきありがとうございます！…励みになります！！`）が免責事項の直前に装飾なし段落で含まれているか
- [ ] **記事末尾**: 挨拶文と免責事項の順序が「挨拶文 → 免責事項」になっているか
