...
> **⚠️ リポジトリ混同注意**: このファイルの例示コードに登場する `YH-05/finance` は**金融ニュース追跡専用リポジトリ**です。コードプロジェクトの Issue（実装タスク、バグ修正等）は `YH-05/note-finance` に登録すること。Issue 作成前に必ず `git remote get-url origin` で対象リポジトリを確認すること。
