...
| news_json_path | No | 単一 JSON ファイルパス（指定時はパターン A） |
| news_json_dir | No | ディレクトリパス（指定時はパターン A；期間内全ファイルをマージ） |
| project_number | No | GitHub Project 番号（デフォルト: 15；パターン B のみ使用） |

## 処理分岐

- **`news_json_path` または `news_json_dir` が指定された場合 → パターン A（ローカル JSON）**
- **いずれも省略された場合 → パターン B（従来の GitHub Project フロー）**

## パターン A: ローカル JSON フロー
...
## パターン B: GitHub Project フロー（従来フロー・変更なし）
...
## GitHub Project データ取得（パターン B のみ）
...
| エラー | 対処 |
|--------|------|
| GitHub CLI エラー（パターン B） | `gh auth status` で認証確認、エラー報告 |
| Project 未検出（パターン B） | Project 番号を確認、エラー報告 |
| 入力ファイル/ディレクトリが見つからない（パターン A） | パスを確認してエラー報告 |
| convert_scraped_news.py 失敗（パターン A） | 終了コードとエラー出力をリーダーに通知 |
