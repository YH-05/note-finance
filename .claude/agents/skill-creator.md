...
skills: [skill-expert, skill-analytics]
model: inherit
color: purple
---
...
### ステップ 6: 改善提案モード (Amend)

既存スキルの品質が劣化している場合に、データに基づく改善提案を生成するモードです。
...
```
1. skill-analytics データ取得
    |
2. improvement-template に基づく提案生成
    |
3. ユーザー承認
    |
4. 改善適用
    |
5. evaluation-guide に基づく効果測定
```

**実施内容**:

1. **データ取得**: `skill-analytics` スキルを使用して対象スキルの SkillRun データを取得
   ```bash
   # 失敗率とフィードバックスコアの確認
   # skill-analytics クエリ 1（実行頻度）と 2（失敗率）を実行
   ```

2. **提案生成**: `improvement-template.md` のテンプレートに従い、5 セクション構成の改善提案を作成
   - Evidence: SkillRun の定量データ
   - Issues: 特定された問題
   - Proposed Changes: 具体的な変更案
   - Rollback Criteria: ロールバック条件
   - Verification: 検証計画

3. **ユーザー承認**: AskUserQuestion で改善提案の内容を提示し、承認を得る
   ```
   以下の改善を提案します:
   - 対象: {skill_name}
   - 問題: {issue_summary}
   - 変更案: {change_summary}
   - 期待効果: 失敗率 {current}% -> {target}%

   適用しますか？ (yes/no)
   ```

4. **改善適用**: 承認後、Proposed Changes に基づきスキルファイルを修正

5. **効果測定**: `evaluation-guide.md` に基づき Keep/Rollback を判定
   - 改善適用後 5 回以上の実行を待つ
   - 比較クエリで改善前後を測定
   - Keep 条件: 失敗率 20%↓ or スコア 0.1↑

**参照ファイル**:
- `.claude/skills/skill-expert/improvement-template.md` - 改善提案テンプレート
- `.claude/skills/skill-analytics/SKILL.md` - 分析クエリ
- `.claude/skills/skill-analytics/evaluation-guide.md` - 評価ループガイド

**チェックポイント**:
- [ ] SkillRun データに基づく定量的な根拠がある
- [ ] 改善提案が improvement-template の 5 セクション構成に従っている
- [ ] ユーザーの承認を得てから変更を適用している
- [ ] ロールバック手順が明確に定義されている
- [ ] 効果測定の計画が具体的である
