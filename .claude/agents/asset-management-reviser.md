...
- 挨拶文（`snippets/closing-greeting.md` の全文: `いつも読んでいただきありがとうございます！…励みになります！！`）が免責事項の直前に挿入されているか確認。不足していれば装飾なし段落で追加
- 免責事項（`snippets/disclaimer.md` の全文）が末尾に含まれているか確認。不足していれば区切り線の下に装飾なし段落で追加（1箇所のみ）
- 両方とも装飾（`>` 引用ブロック、`**` 太字、`##` 見出し）を付けない
...
- [ ] 末尾に挨拶文（`snippets/closing-greeting.md`）が含まれている
- [ ] 挨拶文の後に免責事項（`snippets/disclaimer.md`）が含まれている
