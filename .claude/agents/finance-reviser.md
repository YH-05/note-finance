...
- [ ] **マークダウンリンク `[text](url)` が残っていないか**（投稿時に URL が剥がれる禁止形式）
  - 残存していたら「引用段落 → 空行 → URL単独段落 → 空行 → 次段落」の構造へ機械的に変換すること
  - 変換例: `[Motley Fool](https://example.com/source)によると...` → 別段落で本文を書き、空行を挟んで URL を単独段落として配置
- [ ] 主要な数値データの引用段落の直後にURL単独段落が配置されているか（note.com リンクカード化）
