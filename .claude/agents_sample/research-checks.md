...
description: 記事草稿の品質問題を指摘し checks.json 形式で出力するエージェント
input: claims.json + 記事草稿
output: checks.json
...
**発生条件**:
- 入力ファイル（claims.json, first_draft.md）が存在しない
- ファイルの読み込み権限がない
...
💡 対処法:
- ファイルパスが正しいか確認してください
- research-claims, edit-article-writer が正常に完了しているか確認してください
```

**対処法**:
1. ファイルパスを確認
2. research-claims, edit-article-writer が正常に完了しているか確認
3. ファイルのアクセス権を確認
...
- スキーマ定義（data/schemas/checks.schema.json）を確認してください
```
...
ファイル: articles/{article_id}/02_edit/checks.json
エラー詳細: {error_message}
