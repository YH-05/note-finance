"""T. Rowe Price scraper implementation.

Scrapes market insights and research from T. Rowe Price.

Classes
-------
TRowePriceScraper
    Concrete HTML scraper for T. Rowe Price publications.
...
logger = get_logger(__name__, module="t_rowe_price_scraper")
...
class TRowePriceScraper(HtmlReportScraper):
    """Scraper for T. Rowe Price publications.

    Fetches the T. Rowe Price insights page and extracts report metadata
    for market outlook and investment commentary.

    Attributes
    ----------
    listing_url : str
        URL of the T. Rowe Price insights page.
    ...
    listing_url: ClassVar[str] = (
        "https://www.troweprice.com/personal-investing/resources/insights/global-markets-weekly-update"
    )
    article_selector: ClassVar[str] = (
        "div.article-list a, div.insight-card a, a.content-link"
    )
    ...
    def source_key(self) -> str:
        """Unique identifier for T. Rowe Price.
    ...
    def source_config(self) -> SourceConfig:
        """Configuration for T. Rowe Price source.
    ...
    def parse_listing_item(
        ...
        logger.debug(
            "Parsed T. Rowe Price listing item",
            title=title,
            url=url,
            pdf_url=pdf_url,
        )
    ...
    async def extract_report(self, meta: ReportMetadata) -> ScrapedReport | None:
        """Extract report content from T. Rowe Price.
