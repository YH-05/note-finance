"""YAML configuration loader with Pydantic validation for pdf_pipeline.
...
def load_config(path: Path) -> PipelineConfig:
    ...
    contents against ``PipelineConfig``.
    ...
    >>> from pathlib import Path
    >>> config = load_config(Path("data/config/pdf-pipeline-config.yaml"))
    >>> len(config.input_dirs) > 0
    True
    """
    logger.debug("Loading pipeline configuration", path=str(path))
    ...
    logger.info(
        ...
        path=str(path),
        input_dir_count=len(config.input_dirs),
