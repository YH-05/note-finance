# 【🇺🇸米株決算】NVIDIA（NVDA）Q1 2027 決算レビュー：「完璧な決算」で株価が下がる理由
...
それでも株価が下がった理由はシンプルで、**市場の「ささやき値（whisper number）」がガイダンスをすでに上回っていた**からです。

ここで「ささやき値」とは、公式のアナリストコンセンサスとは別に、機関投資家やヘッジファンドの間で実質的な期待値として共有されている非公式の予測値を指します。決算発表前にこのささやき値が市場予想を大きく上回ると、たとえ公式コンセンサスをビートしても「想定の範囲内」と見なされ、株価は下落することがあります。

今回も同様の構造でした。アナリストコンセンサスは$85〜87Bでしたが、ささやき値は$92〜95Bレンジに到達していたとされます。NVIDIAは過去3四半期連続、ビートしても株価が翌日に下落しています。「完璧を織り込んだ市場は、優れた決算では満足しない」という構造です。
...
> ※ 表中の数値は経営陣カンファレンスコールでの開示ベースであり、公式セグメント開示の合計（$81.6B）と端数差（約$0.2B）があります。経営陣カテゴリは Data Center内訳の集計のため、Edge Computingとの境界に若干の重なりがあります。

ハイパースケーラー（CSP）依存度はData Centerの約50%にまで低下し、残り50%を**ACIE（AI Cloud・Industrial・Enterprise）**が占める構造になりました。とくに**Sovereign AI**（政府発注のAI factory）が今後の成長ドライバーとして強調されており、中東・欧州・日本などで本契約が進んでいます。
...
粗利益率の意味をさらに掘り下げると、Blackwellは前世代Hopperと比較して**チップ当たり製造コストが大幅に上昇**しています（パッケージング・HBM3eメモリ・CoWoS製造プロセスの複雑化のため）。にもかかわらず粗利益率が75%水準で維持されているということは、NVIDIAが製造コスト上昇分を販売価格に転嫁できているという意味で、**「ASP（平均販売価格）の上昇余地がまだ残っている」**ことを示唆します。これはモデル開発側の「精度・速度競争」が続く限り、ハイエンドGPUのプレミアム需要が剥がれにくいという構造的優位を意味します。
...
![プレビュー想定 vs Q1 FY2027実績](images/table_preview_vs_actual.png)
...
特に注意すべきは「ささやき値」の動向です。Q2は市場予想がさらに切り上がり、$95〜100Bを織り込む可能性があります。仮にNVIDIAが$95Bという驚異的な実績を出しても、ささやき値が$100Bだった場合は再び株価が下落するリスクがあります。
